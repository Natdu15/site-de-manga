...
// Charger les données depuis RESULTATS.json
async function chargerVotes() {
  try {
    const response = await fetch("RESULTATS.json"); // Chargement local
    if (!response.ok) {
...
function afficherPodium(containerId, votesObj, type) {
  ...
  const top3 = Object.entries(votesObj)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3);
